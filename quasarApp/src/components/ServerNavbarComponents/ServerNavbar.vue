      ...
      <q-expansion-item dense dense-toggle expand-separator icon="priority_high" label="Invites">
        <q-list dense padding>
          <template v-for="server in channelsInvites" :key="server.id">
            <Server type="invite" :name="server.name" :channel_id="server.id"></Server>
          </template>
        </q-list>
      </q-expansion-item>
      <q-expansion-item dense dense-toggle expand-separator icon="tag" label="Channels">
        <q-list dense padding>
          <template v-for="server in channelsJoined" :key="server.id">
            <Server :private="server.private" :name="server.name" :channel_id="server.id"></Server>
          </template>
        </q-list>
      </q-expansion-item>
      <q-expansion-item dense dense-toggle expand-separator icon="travel_explore" label="Public Channels">
        <q-list dense padding>
          <template v-for="server in publicChannels" :key="server.id">
            <Server :name="server.name" :channel_id="server.id"></Server>
          </template>
        </q-list>
      </q-expansion-item>
