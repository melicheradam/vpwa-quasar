...
interface ConfigWrapper extends AxiosRequestConfig{
  dontTriggerLogout: boolean
}
...
api.interceptors.response.use(
  ...
  (error: AxiosError): Promise<AxiosError> => {
    ...
    if (error.response !== undefined)
      if (error.response.status === 401 && !(error.response.config as ConfigWrapper).dontTriggerLogout) {
        authManager.logout()
      }
